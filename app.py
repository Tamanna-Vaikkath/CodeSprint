...
# ----------------------------------------------------------------------------
# View: Artificial Intelligence & Machine Learning (separate tab)
# ----------------------------------------------------------------------------
def render_aiml_problems():
    st.title(AIML_CATEGORY)

    questions = [q for q in load_questions() if q.get("category") == AIML_CATEGORY]
    if not questions:
        st.info("No AI/ML questions found in the bank.")
        return

    fc1, fc2, fc3 = st.columns([2, 1, 1])
    with fc1:
        search = st.text_input("Search by title or tag", "", key="aiml_search")
    with fc2:
        diff_filter = st.multiselect(
            "Difficulty", ["Easy", "Medium", "Hard"], default=[], key="aiml_diff"
        )
    with fc3:
        all_tags = sorted({t for q in questions for t in q.get("tags", [])})
        tag_filter = st.multiselect("Tags", all_tags, default=[], key="aiml_tags")
    ...
    st.caption(f"Showing {len(filtered)} of {len(questions)} MCQs")
    st.divider()

    page_items = _paginate(filtered, "aiml")
    _render_question_table(page_items, origin="aiml")
...
def render_submissions():
    ...
    # --- overall summary ---
    all_subs = [(qid, s) for qid, subs in st.session_state.submissions.items() for s in subs]
    total_attempts = len(all_subs)
    solved_qids = {qid for qid, subs in st.session_state.submissions.items()
                   if any(s["passed_all"] for s in subs)}
    mcq_subs = [s for _, s in all_subs if s.get("language") == "MCQ"]
    mcq_points_earned = sum(s.get("points", 0) for s in mcq_subs)
    mcq_points_possible = sum(s.get("points_possible", 0) for s in mcq_subs)

    sc1, sc2, sc3 = st.columns(3)
    sc1.metric("Total submissions", total_attempts)
    sc2.metric("Questions solved", len(solved_qids))
    if mcq_points_possible:
        sc3.metric("MCQ score (latest per Q)", f"{mcq_points_earned}/{mcq_points_possible} pts")
    st.divider()

    for qid, subs in st.session_state.submissions.items():
        q = questions_by_id.get(qid, {"title": qid, "difficulty": "?", "category": None})
        cat = q.get("category")
        badge = f"  ·  {cat}" if cat else ""
        st.markdown(f"### {q['title']}{badge}")
        for s in reversed(subs):
            status = "Accepted" if s["passed_all"] else "Not solved"
            status_color = "#1DA362" if s["passed_all"] else "#D64545"
            if s.get("language") == "MCQ":
                st.markdown(
                    f"<span style='color:{status_color}; font-weight:700;'>{status}</span> "
                    f"· {s.get('points', 0)}/{s.get('points_possible', 1)} pts "
                    f"({s['score_pct']}%) · {s['timestamp']}",
                    unsafe_allow_html=True,
                )
                with st.expander("View answer & explanation"):
                    st.write(f"**Your answer:** {s['selected_letter']}. {s['selected_text']}")
                    st.write(f"**Correct answer:** {s['correct_letter']}. {s['correct_text']}")
                    if s.get("explanation"):
                        st.caption(f"Explanation: {s['explanation']}")
            else:
                st.markdown(
                    f"<span style='color:{status_color}; font-weight:700;'>{status}</span> "
                    f"· {s['language']} · {s['passed_count']}/{s['total_count']} "
